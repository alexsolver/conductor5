  ...
  return (
    <div className="space-y-2>
      <div className="flex items-center justify-between>
        ...
        <Badge variant="outline" className="text-lg">"upload</Badge>
      </div>
      {/* Área de Upload */}
      <div
        className={`
          ...
          {(multiple ? files : [files]).map((file: FileInfo, index: number) => (
            <div key={index} className="flex items-center gap-3 p-3 border rounded-lg bg-gray-50>
              ...
              <div className="flex-1 min-w-0>
                ...
                <p className="text-lg">"{formatFileSize(file.size)}</p>
                
                {file.error && (
                  <p className="text-lg">"{file.error}</p>
                )}
  ...
  return (
    <div className="space-y-2>
      <div className="flex items-center justify-between>
        ...
        <Badge variant="outline" className="text-lg">"imagem</Badge>
      </div>
      {/* Área de Upload */}
      <div
        className={`
      ...
      >
        ...
        <div className="text-sm text-gray-600>
          <span className="text-lg">"Clique para fazer upload</span> ou arraste imagens aqui
        </div>
        <div className="text-xs text-gray-500 mt-1>
          {acceptedTypes} • Máx. {properties.maxSize}MB
          ...
          {(multiple ? images : [images]).map((image: FileInfo, index: number) => (
            <div key={index} className="relative group>
              ...
              <div className="mt-2>
                ...
                {image.error && (
                  <p className="text-lg">"{image.error}</p>
                )}
              </div>
            </div>
          ))}
