...
const linkTicketsMutation = useMutation({
    mutationFn: async (data: { ticketIds: string[]; relationshipType: string }) => {
      const promises = data.ticketIds.map(async (targetTicketId) => {
        const response = await apiRequest("POST", `/api/tickets/${currentTicket!.id}/relationships`, {
          targetTicketId,
          relationshipType: data.relationshipType,
          description: `Vinculado via modal - Tipo: ${data.relationshipType}`,
        });
        return response.json();
      });
    ...
    onSuccess: () => {
      ...
      // Invalidar também o cache geral de relacionamentos
      queryClient.invalidateQueries({ 
        queryKey: ["/api/ticket-relationships", currentTicket?.id, "relationships"] 
      });

      // Forçar refetch imediato dos relacionamentos
      queryClient.refetchQueries({
        queryKey: ["/api/ticket-relationships", currentTicket?.id, "relationships"]
      });

      // Invalidar cache de batch relationships se existir
      queryClient.invalidateQueries({
        predicate: (query) => 
          query.queryKey[0] === '/api/tickets' && 
          query.queryKey.includes('batch-relationships')
      });
