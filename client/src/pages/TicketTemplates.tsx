...
const templateFormSchema = z.object({
  ...
  estimatedHours: z.preprocess((val) => Number(val), z.number().min(0)),
...
export default function TicketTemplates() {
  ...
  const { data: templatesResponse, isLoading: templatesLoading } = useQuery({
    ...
    queryFn: async () => {
      console.log(
        "🔍 [TEMPLATES-QUERY] Fetching templates for company:",
        selectedCompany,
      );
      const endpoint =
        selectedCompany === "all"
  ...
  const { data: templateForEdit } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await apiRequest(
        "GET",
        `/api/ticket-templates/${editingTemplate.id}`,
      );
      return response.json();
    },
  ...
  const templates = React.useMemo(() => {
    ...
    if (templatesResponse.success && templatesResponse.data?.templates) {
      console.log(
        "✅ [TEMPLATES-PROCESSING] Found templates:",
    ...
    if (Array.isArray(templatesResponse)) {
      console.log(
        "✅ [TEMPLATES-PROCESSING] Direct array response:",
        templatesResponse.length,
      );
    ...
    if (templatesResponse.data && Array.isArray(templatesResponse.data)) {
      console.log(
        "✅ [TEMPLATES-PROCESSING] Root data array:",
    ...
    if (templatesResponse.success === false) {
      console.log(
        "❌ [TEMPLATES-PROCESSING] API error response:",
        templatesResponse.errors,
      );
      return [];
    }

    console.log(
      "❌ [TEMPLATES-PROCESSING] Unexpected response structure:",
      templatesResponse,
    );
    return [];
  }, [templatesResponse]);
  ...
  const categories = React.useMemo(() => {
    ...
    if (categoriesResponse.success && categoriesResponse.data?.categories) {
      rawCategories = Array.isArray(categoriesResponse.data.categories)
        ...
        : [];
      console.log(
        "✅ [CATEGORIES-PROCESSING] Found categories:",
    ...
    // Handle direct array response
      ...
      console.log(
        "✅ [CATEGORIES-PROCESSING] Direct array response:",
        rawCategories.length,
      );
    }
    // Handle direct array at root level
      ...
      console.log(
        "✅ [CATEGORIES-PROCESSING] Root array response:",
        rawCategories.length,
      );
    }
    // Extract unique categories from templates if available
      ...
      rawCategories = templates
        ...
        .filter(Boolean);
      console.log(
        "✅ [CATEGORIES-PROCESSING] Extracted from templates:",
        rawCategories.length,
      );
    } else {
    ...
    console.log(
      "🔧 [CATEGORIES-PROCESSING] Unique categories after deduplication:",
  ...
  const createTemplateMutation = useMutation({
    mutationFn: async (data: TemplateFormData) => {
      ...
      const payload = {
        ...
        isActive: true,
        usageCount: 0,
        tags: data.defaultTags
          ? data.defaultTags.split(",").map((t) => t.trim())
          : [],
  ...
  const deleteTemplateMutation = useMutation({
    mutationFn: (id: string) =>
      apiRequest("DELETE", `/api/ticket-templates/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["ticket-templates", selectedCompany],
      });
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-blue-100 mt-2">
                Sistema completo de templates com Clean Architecture e análise
                inteligente
              </p>
            </div>
            <Button
                        ...
                        <div className="flex items-center gap-4 flex-1">
                          ...
                          <div className="flex-1">
                            <div className="flex items-center gap-3">
                              ...
                              <Badge
                                className={`${getPriorityColor(template.priority)} border-0`}
                              >
            ...
            <div className="bg-white rounded-lg shadow-sm border">
              <div className="border-b bg-gradient-to-r from-gray-50 to-blue-50 p-4">
                ...
                <p className="text-gray-600 mt-1">
                  Crie templates usando drag-and-drop com campos do sistema e
                  customizados.
                </p>
              </div>
              <div className="p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="bg-gradient-to-r from-purple-50 to-blue-50 p-4 rounded-lg">
                    ...
                    <p className="text-sm text-gray-600">
                      Crie templates específicos com campos customizados para
                      esta empresa.
                    </p>
                  </div>
                  <div className="bg-gradient-to-r from-blue-50 to-cyan-50 p-4 rounded-lg">
                    ...
                    <p className="text-sm text-gray-600">
                      Configure regras de aprovação baseadas no perfil da
                      empresa.
                    </p>
                  </div>
                </div>
