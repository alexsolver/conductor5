  ...
  <div className="space-y-8">
    ...
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {["Support", "Performance", {t('Dashboards.error')}, "Compliance"].map((category) => (
...
const ZendeskDashboardCard = ({ dashboard, onRefresh }: { dashboard: Dashboard; onRefresh: () => void }) => {
  ...
  const duplicateDashboard = useMutation({
    ...
    onSuccess: () => {
      toast({ 
        title: {t('Dashboards.dashboardDuplicated')},
  ...
  const deleteDashboard = useMutation({
    ...
    onSuccess: () => {
      toast({ 
        title: {t('Dashboards.dashboardDeleted')},
    ...
    try {
      ...
      if (navigator.clipboard) {
        ...
        toast({ 
          ...
          description: {t('Dashboards.error')}
        });
      } else {
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Textarea 
                          {...field} 
                          placeholder={t('Dashboards.error')}
                          rows={3}
                        />
                      </FormControl>
                      <FormMessage />
        ...
        <Tabs value={activeView} onValueChange={setActiveView} className="mb-8">
          ...
          <TabsContent value="templates" className="mt-6">
            <ZendeskTemplateShowcase onSelectTemplate={(template) => {
              console.log({t('Dashboards.error')}, template);
            }} />
          </TabsContent>
