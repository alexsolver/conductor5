...
if (!schema.scheduleTemplates || !schema.workSchedules || !schema.users) {
  throw new Error(
    "Critical schema tables are missing. Check @shared/schema imports.",
  );
}
...
console.log(
  "🔥 [1QA-FINAL] NEON COMPLETAMENTE REMOVIDO DA CODEBASE - PostgreSQL (pg) driver ativo",
);
console.log(
  "✅ [1QA-SUCCESS] @neondatabase/serverless eliminado, node-postgres implementado",
);
...
export const schemaManager = {
  ...
  async runTenantMigrations(tenantId: string) {
    try {
      console.log(
        `🔄 [SCHEMA-MANAGER] Running tenant migrations for: ${tenantId}`,
      );
      const { MigrationManager } = await import(
        "./migrations/pg-migrations/config/migration-manager"
      );
      ...
      console.log(
        `✅ [SCHEMA-MANAGER] Tenant migrations completed for: ${tenantId}`,
      );
      return true;
    } catch (error) {
      console.error(
        `❌ [SCHEMA-MANAGER] Tenant migration error for ${tenantId}:`,
        error,
      );
      return false;
    }
  ...
  async validateTenantSchema(tenantId: string): Promise<boolean> {
    try {
      ...
      const customerTables = customerModuleTables.rows.map(
        (row) => row.table_name,
      );
      ...
      const customerSoftDeleteCoverage = customerSoftDeleteCheck.rows.filter(
        (row) => row.has_is_active,
      ).length;

      // ✅ CUSTOMER MODULE VALIDATION CRITERIA - ENHANCED
      const customerModuleValid =
        customerTables.length >= 6 &&
        customerFieldsOk &&
        beneficiariesFieldsOk &&
        customerSoftDeleteCoverage >= 4 &&
        foreignKeysOk;
      const overallValid = tableCount >= 60 && customerModuleValid;

      console.log(`🏢 Customer Module validated for ${tenantId}:`);
      console.log(
        `   📋 Customer tables: ${customerTables.length}/8 (${customerTables.join(", ")})`,
      );
      console.log(
        `   🔧 Customer fields: ${customerFieldsOk ? "OK" : "MISSING"}`,
      );
      console.log(
        `   🏷️ Beneficiaries fields: ${beneficiariesFieldsOk ? "OK" : "MISSING"}`,
      );
      console.log(
        `   🔗 Foreign keys: ${foreignKeysOk ? "OK" : "MISSING"} (${foreignKeyCheck.rows.length} found)`,
      );
      console.log(
        `   🗑️ Soft delete coverage: ${customerSoftDeleteCoverage}/5`,
      );
      console.log(
        `   ✅ Total tables: ${tableCount} - ${overallValid ? "VALID" : "INVALID"}`,
      );
      ...
      if (!customerModuleValid) {
        ...
        console.log(
          `❌ Missing customer tables: ${expectedCustomerTables.filter((t) => !customerTables.includes(t))}`,
        );
        console.log(
          `❌ Missing foreign keys - expected relationships in tickets, memberships, and beneficiaries`,
        );
      }
      ...
      console.error(
        `❌ Customer Module schema validation failed for ${tenantId}:`,
        error,
      );
      return false;
    }
  ...
  async ensureTenantTables(tenantId: string) {
    ...
    console.log(
      `✅ Tenant tables validation for ${tenantId} - using existing tables`,
    );
    return true;
  },
  ...
  async createTenantSchema(tenantId: string): Promise<boolean> {
    ...
    try {
      ...
      console.log(
        `✅ [SCHEMA-MANAGER] Schema created successfully: ${schemaName}`,
      );
      ...
      try {
        ...
        console.log(
          `✅ [SCHEMA-MANAGER] Tables initialized for schema: ${schemaName}`,
        );
      } catch (tableError) {
        console.error(
          `❌ [SCHEMA-MANAGER] Failed to initialize tables for ${schemaName}:`,
          tableError,
        );
        throw new Error(
      ...
      console.error(
        `❌ [SCHEMA-MANAGER] Error creating tenant schema ${schemaName}:`,
        error,
      );
      throw error;
    }
