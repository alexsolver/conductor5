...
/**
 * Generic integration test endpoint - works for all integrations
 */
// POST /api/tenant-admin/integrations/:integrationId/test
router.post('/integrations/:integrationId/test', requirePermission(Permission.TENANT_MANAGE_SETTINGS), async (req: AuthorizedRequest, res) => {
  console.log(`🔍 [TENANT-ADMIN-TEST] Starting test for integration: ${req.params.integrationId}`);
  try {
    ...
    console.log(`🔍 [TENANT-ADMIN-TEST] Getting config for ${integrationId} and tenant ${tenantId}`);
    
    // Get integration configuration
    const config = await storageSimple.getTenantIntegrationConfig(tenantId, integrationId);
    ...
    if (integrationId === 'telegram') {
      console.log(`🔍 [TELEGRAM-TEST-TENANT] Testing Telegram integration`);
      
      if (!config?.config) {
        testResult = {
          success: false,
          error: 'Configuração do Telegram não encontrada. Configure a integração primeiro.',
          details: {
        ...
        const telegramConfig = config.config;
        const hasBotToken = !!telegramConfig.telegramBotToken;
        const hasChatId = !!telegramConfig.telegramChatId;
        ...
        if (!hasBotToken) {
          ...
          testResult = {
            success: false,
            error: 'Chat ID do Telegram não configurado',
            details: {
              status: 'Configuração incompleta', 
          ...
          // Test Telegram API
          try {
            const telegramApiUrl = `https://api.telegram.org/bot${telegramConfig.telegramBotToken}/getMe`;
            console.log(`🔍 [TELEGRAM-TEST-TENANT] Testing bot token with Telegram API...`);
            ...
            if (response.ok) {
              ...
              if (data.ok && data.result) {
                testResult = {
                  ...
                  error: '',
                  details: {
                    status: 'Bot Telegram verificado e funcionando',
                    botInfo: {
                      id: data.result.id,
                ...
                await storageSimple.updateTenantIntegrationStatus(tenantId, integrationId, 'connected');
              } else {
                testResult = {
                  success: false,
                  error: 'Token do bot inválido: ' + (data.description || 'Erro desconhecido'),
                  details: {
                    status: 'Token inválido',
                    apiError: data.description || 'Erro desconhecido'
                  }
                };
              }
            } else {
              let errorMsg = 'Erro na API do Telegram';
              if (response.status === 401) {
                errorMsg = 'Token do bot inválido';
              } else if (response.status === 404) {
                errorMsg = 'Bot não encontrado';
              }
            ...
            console.log(`❌ [TELEGRAM-TEST-TENANT] API error:`, (apiError as Error).message);
            testResult = {
              success: false,
              error: (apiError as Error).name === 'AbortError' ? 
                'Timeout ao conectar com API do Telegram' : 
                'Erro de conectividade: ' + (apiError as Error).message,
              details: {
    ...
    } else if (integrationId === 'gmail-oauth2') {
      // Forward to existing Gmail OAuth2 test implementation
      if (!config?.config) {
        testResult = {
          success: false,
      ...
      testResult = {
        success: false,
        error: `Teste não implementado para a integração: ${integrationId}`,
        details: {
          integrationId,
          supportedIntegrations: ['telegram', 'gmail-oauth2']
        }
      };
    }

    console.log(`🔍 [TENANT-ADMIN-TEST] Final result for ${integrationId}:`, JSON.stringify(testResult, null, 2));
