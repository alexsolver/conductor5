...
export class DrizzleUserSkillRepository implements IUserSkillRepository {
  ...
  async getExpiredCertifications(): Promise<Array<{
    userId: string;
    userName: string;
    skillName: string;
    certificationName: string;
    ...
    try {
      const results = await db.select({
        ...
        userName: sql<string>`'Sistema'`, // Simplified until users table is properly joined
        skillName: technicalSkills.name,
        certificationName: userSkills.certificationName,
        expirationDate: userSkills.expiresAt,
      })
      .from(userSkills)
      .innerJoin(technicalSkills, eq(userSkills.skillId, technicalSkills.id))
      .where(and(
        eq(userSkills.isActive, true),
        isNotNull(userSkills.expiresAt),
        sql`${userSkills.expiresAt} < NOW()`
      ))
      .orderBy(desc(userSkills.expiresAt));

      return results.map(r => ({
        userId: r.userId,
        userName: r.userName,
        skillName: r.skillName,
        certificationName: r.certificationName || 'Certificação',
  ...
  async getExpiringCertifications(daysAhead: number = 30): Promise<Array<{
    userId: string;
    userName: string;
    skillName: string;
    certificationName: string;
    ...
    try {
      ...
      .where(and(
        ...
        isNotNull(userSkills.expiresAt),
        sql`${userSkills.expiresAt} > NOW()`,
        sql`${userSkills.expiresAt} <= NOW() + INTERVAL '${daysAhead} days'`
      ))
      ...
      return results.map(r => ({
        ...
        certificationName: r.certificationName || 'Certificação',
        expirationDate: r.expirationDate!,
        daysUntilExpiration: Math.ceil((r.expirationDate!.getTime() - Date.now()) / (1000 * 60 * 60 * 24)),
      }));
    } catch (error) {
