...
cleanCompaniesRouter.get(
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      const companies = await findCompanyUseCase.findByStatus(
        status,
        req.user?.tenantId || "",
      );
      ...
      res.status(400).json({
        ...
        message:
          error instanceof Error
            ? error.message
            : "Failed to find companies by status",
...
cleanCompaniesRouter.get(
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      const companies = await findCompanyUseCase.findBySize(
        size,
        req.user?.tenantId || "",
      );
      ...
      res.status(400).json({
        ...
        message:
          error instanceof Error
            ? error.message
            : "Failed to find companies by size",
...
cleanCompaniesRouter.get(
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      const companies = await findCompanyUseCase.findBySubscription(
        tier,
        req.user?.tenantId || "",
      );
      ...
      res.status(400).json({
        ...
        message:
          error instanceof Error
            ? error.message
            : "Failed to find companies by subscription",
...
cleanCompaniesRouter.get(
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      const companies = await findCompanyUseCase.findByIndustry(
        industry,
        req.user?.tenantId || "",
      );
      ...
      res.status(400).json({
        ...
        message:
          error instanceof Error
            ? error.message
            : "Failed to find companies by industry",
    ...
    try {
      ...
      res.status(400).json({
        ...
        message:
          error instanceof Error
            ? error.message
            : "Failed to find companies by location",
...
cleanCompaniesRouter.get(
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      const companies = await findCompanyUseCase.findRecentCompanies(
        req.user?.tenantId || "",
        days,
      );
      ...
      res.status(400).json({
        ...
        message:
          error instanceof Error
            ? error.message
            : "Failed to find recent companies",
...
cleanCompaniesRouter.get(
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      const company = await findCompanyUseCase.findByCNPJ(
        cnpj,
      ...
      res.status(400).json({
        success: false,
        message:
          error instanceof Error ? error.message : "Failed to check CNPJ",
...
cleanCompaniesRouter.get(
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      const company = await findCompanyUseCase.findByEmail(
        email,
        req.user?.tenantId || undefined,
      );
...
router.get("/", jwtAuth, async (req: any, res: Response) => {
  try {
    ...
    const { db: tenantDb, schema: tenantSchema } =
      await schemaManager.getTenantDb(tenantId);
    ...
    const allCompanies = await tenantDb
      ...
      .from(companies)
      .where(
        and(eq(companies.tenantId, tenantId), eq(companies.isActive, true)),
      )
