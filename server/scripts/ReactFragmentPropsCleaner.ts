...
export class ReactFragmentPropsCleaner {
  ...
  private checkReactFragmentIssues(line: string, lineNumber: number, filePath: string, fullContent: string): void {
  ...
  private async applyFixes(): Promise<void> {
    ...
    const filesByPath = new Map<string, ReactIssue[]>();
  ...
  private async fixFile(filePath: string, issues: ReactIssue[]): Promise<void> {
    try {
