...
import customFieldRoutes from './modules/custom-fields/routes-working.js';
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // ✅ LEGACY ROUTE ELIMINATED - Using updated implementation below per 1qa.md patterns
  ...
  app.post('/api/saas-admin/integrations/:integrationId/test', jwtAuth, async (req: AuthenticatedRequest, res) => {
    try {
      ...
      if (!configResult.rows[0]?.config) {
        return res.json({
          success: false,
          message: 'Configuração não encontrada. Configure a integração primeiro.',
      ...
      try {
        ...
        console.error(`❌ [SAAS-ADMIN-TEST] Test failed for ${integrationId}:`, error);
        res.json({
          success: false,
