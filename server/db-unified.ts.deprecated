...
export class UnifiedDatabaseManager {
  ...
  async recreateAllTables(): Promise<void> {
    console.log('🔄 Starting complete table recreation...');
    ...
    try {
      ...
      console.log('✅ Complete table recreation completed successfully!');
    } catch (error) {
  ...
  private async dropAllTenantSchemas(): Promise<void> {
    console.log('🗑️ Dropping all existing tenant schemas...');
  ...
  private async createPublicTables(): Promise<void> {
    ...
    try {
      ...
      console.log('✅ Public tables created successfully');
    } catch (error) {
  ...
  private async recreateAllTenantSchemas(): Promise<void> {
    console.log('🏢 Recreating all tenant schemas...');
    ...
    console.log('✅ All tenant schemas recreated successfully');
  }

  async createTenantSchema(tenantId: string): Promise<void> {
    console.log(`🏗️ Creating tenant schema for: ${tenantId}`);
  ...
  private async insertSampleData(schemaName: string, tenantId: string): Promise<void> {
    try {
      ...
      await db.execute(sql.raw(`
        ...
        VALUES 
          ('${tenantId}', 'Sede Principal', 'Av. Paulista, 1000', 'São Paulo', 'SP', 'Brasil'),
          ('${tenantId}', 'Filial Rio', 'Rua Copacabana, 500', 'Rio de Janeiro', 'RJ', 'Brasil'),
          ('${tenantId}', 'Centro de Distribuição', 'Rod. Anhanguera, km 25', 'Osasco', 'SP', 'Brasil')
      `));
      ...
      await db.execute(sql.raw(`
        ...
        VALUES 
          ('${tenantId}', 'João', 'Silva', 'joao.silva@exemplo.com', '(11) 99999-1111', '123.456.789-01', 'fisica'),
      ...
      await db.execute(sql.raw(`
        ...
        VALUES 
          ('${tenantId}', 'TK-000001', 'Problema com sistema', 'Sistema apresentando lentidão', 'open', 'high'),
          ('${tenantId}', 'TK-000002', 'Solicitação de acesso', 'Novo usuário precisa de acesso', 'open', 'medium')
      `));

      console.log(`✅ Sample data inserted for ${schemaName}`);
    } catch (error) {
