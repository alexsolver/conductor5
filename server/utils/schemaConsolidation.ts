...
export class SchemaConsolidationService {
  ...
  private static async addMissingIndexes(schemaName: string): Promise<void> {
    try {
      ...
      const indexes = [
        { table: 'customers', columns: ['tenant_id', 'email'], name: 'idx_customers_tenant_email' },
        { table: 'customers', columns: ['tenant_id', 'active'], name: 'idx_customers_tenant_active' },
        { table: 'customers', columns: ['documento'], name: 'idx_customers_documento' },
        { table: 'tickets', columns: ['tenant_id', 'status'], name: 'idx_tickets_tenant_status' },
        { table: 'tickets', columns: ['customer_id'], name: 'idx_tickets_customer_id' },
        { table: 'tickets', columns: ['assigned_to_id'], name: 'idx_tickets_assigned_to' },
        { table: 'ticket_messages', columns: ['ticket_id'], name: 'idx_ticket_messages_ticket_id' },
        { table: 'activity_logs', columns: ['tenant_id', 'entity_type', 'entity_id'], name: 'idx_activity_logs_entity' },
        { table: 'favorecidos', columns: ['tenant_id', 'ativo'], name: 'idx_favorecidos_tenant_ativo' },
        { table: 'locations', columns: ['tenant_id'], name: 'idx_locations_tenant_id' },
        { table: 'projects', columns: ['tenant_id', 'status'], name: 'idx_projects_tenant_status' },
        { table: 'user_skills', columns: ['user_id'], name: 'idx_user_skills_user_id' },
      ];
