...
- **Logging**: Winston.
    - **OmniBridge Conversation Logging & Learning System**: Advanced AI agent improvement system with complete conversation tracking, action execution logging, contextual feedback annotations, and continuous learning capabilities. Features include: 5-table schema for comprehensive logging (conversation_logs, conversation_messages, action_executions, feedback_annotations, agent_improvements), API endpoints for accessing conversation history with filters and search, analytics dashboard with agent performance metrics (escalation rate, avg messages/actions, success rate by action type), feedback annotation system allowing users to mark specific messages/actions with ratings, severity, corrective actions, and expected behavior, and agent improvement tracking with version control and rollback capabilities. Enables users to select conversation excerpts and provide guidance to refine AI agent context and behavior over time.
    - **AI Agent Auto-Configuration System (Oct 2025)**: Natural language-to-configuration generation using GPT-4o-mini. Users describe desired agent behavior in plain language, system automatically generates complete configuration including personality, enabled actions, behavior rules, and AI settings. **CORS/Auth Fix**: Public endpoint (`/api/ai-agents/generate-config`) created directly in `server/index.ts` before all authentication middlewares to bypass JWT/tenant validation for OPTIONS preflight and POST requests, enabling seamless frontend integration.
